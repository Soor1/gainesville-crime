// same shell sort as the one in sorting/shellsort.js but slightly modified to count the number of comparisons
...
// funciton to find the k largest elements in an array using shell sort
function kLargestShellSort(v, k, count) {
    shellSort(v, count);

    if (v.length > k) return v.slice(v.length - k, v.length);
    else return v;
}

// the same heapsort as the one in sorting/heapsort.js but slightly modified to count the number of comparisons
